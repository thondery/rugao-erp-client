...
const ACTION_HANDLERS = {
  [types.ADMINS_USER_LIST_BEGIN]: (state, action) => {
    return {
      ...state,
      getListPending: false,
      getListError: -1,
  ...
  [types.ADMINS_USER_LIST_SUCCESS]: (state, action) => {
    ...
    return {
      ...
      getListPending: false,
      listData: data ? data.users : null,
      groups: data ? data.groups : null
    }
  },
  [types.ADMINS_USER_LIST_FAILURE]: (state, action) => {
    const { error } = action
  ...
  [types.ADMINS_USER_EDIT_BEGIN]: (state, action) => {
    return {
      ...state,
      editPending: true,
      editError: -1,
  ...
  [types.ADMINS_USER_EDIT_SUCCESS]: (state, action) => {
    const { payload } = action
    const { data, status } = payload
    return {
      ...state,
  ...
  [types.ADMINS_USER_EDIT_FAILURE]: (state, action) => {
    const { error } = action
    const status = getStatusError(error)
    return {
      ...state,
  ...
  [types.ADMINS_USER_ADD_BEGIN]: (state, action) => {
    return {
      ...state,
      addPending: true,
      addError: -1,
  ...
  [types.ADMINS_USER_ADD_SUCCESS]: (state, action) => {
    const { payload } = action
    const { data, status } = payload
    return {
      ...state,
  ...
  [types.ADMINS_USER_ADD_FAILURE]: (state, action) => {
    const { error } = action
    const status = getStatusError(error)
    return {
      ...state,
  ...
  [types.ADMINS_USER_REMOVE_BEGIN]: (state, action) => {
    return {
      ...state,
      removePending: true,
      removeError: -1,
  ...
  [types.ADMINS_USER_REMOVE_SUCCESS]: (state, action) => {
    const { payload } = action
    const { data, status } = payload
    return {
      ...state,
  ...
  [types.ADMINS_USER_REMOVE_FAILURE]: (state, action) => {
    const { error } = action
    const status = getStatusError(error)
    return {
      ...state,
...
function updateItem (data, status, listData) {
  if (status.code !== 0) return listData
  const index = _.findIndex(listData, { uid: data.uid })
  listData[index] = data
  return listData
}